...
const FOLK_PRODUCTS = [
  ...
  { id:3, name:"Terracotta Ripple Vase — Trio", cat:"Vase", price:1690, mrp:2290, badge:"New", rating:4.7, reviews:98, difficulty:"Beginner", time:"2.5 hr + dry", includes:"Air-dry clay 1kg • Tools • Paints", image:"https://images.unsplash.com/photo-1578749556568-bc2c40e68b61?w=700&h=700&fit=crop&q=80", desc:"Pinch, coil and carve three ripple vases. No kiln needed — air-dry, sand & paint with our mineral terracotta palette." },
  ...
  { id:10, name:"Hand-Pinched Planter Trio — Terracotta", cat:"Flowerpot", price:1890, mrp:2590, badge:"With Seeds", rating:4.7, reviews:154, difficulty:"Beginner", time:"2 hr + dry", includes:"Stoneware clay • Liners • Seeds", image:"https://images.unsplash.com/photo-1485955900006-10f4d324d411?w=700&h=700&fit=crop&q=80", desc:"No wheel needed — just thumbs. Make three textured planters and plant the included snake plant seeds. Liners included." },
...
function highlightActiveNav() {
  ...
  document.querySelectorAll('#mainNav a').forEach(a => {
    a.classList.remove('bg-ink', 'text-white', 'shadow-sm');
    a.classList.add('hover:bg-white', 'border-transparent');
    
    const navKey = a.dataset.nav;
    let isActive = false;
    
    if (catParam) {
      if (navKey && navKey.toLowerCase() === catParam.toLowerCase()) isActive = true;
    } else if (path === 'index.html' || path === '') {
      if (navKey === 'home') isActive = true;
    } else if (path === 'shop.html') {
      if (navKey === 'all') isActive = true;
    } else if (path === 'workshops.html') {
      if (navKey === 'workshops') isActive = true;
    } else if (path === 'product.html') {
      const pId = Number(urlParams.get('id'));
      const prod = getProducts().find(p => p.id === pId);
      if (prod && navKey && navKey.toLowerCase() === prod.cat.toLowerCase()) {
        isActive = true;
      } else if (navKey === 'all') {
        isActive = true;
      }
    ...
    if (isActive) {
      a.classList.remove('hover:bg-white', 'border-transparent');
      a.classList.add('bg-ink', 'text-white', 'shadow-sm');
    }
  });
  ...
  document.querySelectorAll('#mobileMenu a').forEach(a => {
    ...
    if (catParam && href.includes(`cat=${catParam}`)) {
      a.classList.add('bg-sand', 'font-bold');
    } else if (!catParam && href.includes(path) && path !== '') {
      a.classList.add('bg-sand', 'font-bold');
    }
  });
}
